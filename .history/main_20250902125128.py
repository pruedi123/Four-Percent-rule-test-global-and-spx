...
@st.cache_data
def try_load_default_csv(default_path: str, mtime: float) -> pd.DataFrame | None:
    p = Path(default_path)
    if p.exists():
        df = pd.read_csv(p)
...
# Utility: pick a numeric factor column if no allocation columns exist (e.g., SPX file)
def pick_single_factor_column(df: pd.DataFrame) -> str | None:
    # Prefer a column literally named 'factor' if present
    cols = [c.strip().lower() for c in df.columns]
    if "factor" in cols:
        # return the original-cased name
        return df.columns[cols.index("factor")]
    # Else choose the first numeric column excluding obvious date/text columns
    date_like = {"begin month", "end month", "begin", "end", "date", "month"}
    for c in df.columns:
        if c.strip().lower() in date_like:
            continue
...
# Choose data source
DATA_OPTIONS = {
    "Global (all_factors.csv)": Path(__file__).parent / "all_factors.csv",
    "S&P 500 (spx_factors.csv)": Path(__file__).parent / "spx_factors.csv",
}
data_choice = st.selectbox("Data source", list(DATA_OPTIONS.keys()), index=0)
default_path = DATA_OPTIONS[data_choice]

mtime = default_path.stat().st_mtime if default_path.exists() else 0.0
df = try_load_default_csv(str(default_path), mtime)
active_source = f"{data_choice} ({default_path.name})"

with st.expander("(Optional) Upload a CSV to override the selected file"):
    override_with_upload = st.checkbox("Override with uploaded CSV", value=False)
    uploaded = st.file_uploader("Upload a compatible factors CSV", type=["csv"])
    if override_with_upload and uploaded is not None:
        df = load_from_upload(uploaded)
        active_source = "Uploaded CSV"

if df is None:
    st.error("Couldn't find the selected data file. Place it next to this app or upload a CSV above.")
    st.stop()
...
single_series_col = None
if not alloc_cols:
    # Likely a single-series file (e.g., SPX). Pick a numeric factor column.
    single_series_col = pick_single_factor_column(df)
    if single_series_col is None:
        st.error("No allocation columns found and no numeric factor column detected. Check CSV format.")
        st.stop()
    alloc_cols = [single_series_col]
    default_alloc = single_series_col
else:
    # Prefer spx60e if present (for SPX dataset), else 60e, else first alloc col
    if "spx60e" in df.columns:
        default_alloc = "spx60e"
    elif "60e" in df.columns:
        default_alloc = "60e"
    else:
        default_alloc = alloc_cols[0]

col1, col2, col3, col4 = st.columns([1,1,1,1])
with col1:
    alloc = st.selectbox("Allocation / Factor Column", options=alloc_cols, index=alloc_cols.index(default_alloc))
with col2:
...
# with st.expander("Data inspector (debug)"):
#     st.write("**Active source:**", active_source)
#     st.write("**Selected column:**", alloc)
#     # Show factor values around the chosen start row
#     try:
#         sample_idx = list(range(int(start_row), min(int(start_row) + 5, len(df))))
#         st.write("**Rows shown:**", sample_idx)
#         st.dataframe(df.loc[sample_idx, [c for c in ["begin month", alloc] if c in df.columns]])
#     except Exception as e:
#         st.caption(f"Inspector error: {e}")
#     # Quick fingerprint of the selected factor series
#     try:
#         ser = pd.to_numeric(df[alloc], errors="coerce").dropna()
#         fingerprint = float(ser.head(500).sum())
#         st.caption(f"Series fingerprint (sum of first 500 values): {fingerprint:.6f}")
#     except Exception as e:
#         st.caption(f"Fingerprint error: {e}")
...
if sim_df.empty:
    ...
    # Inline batch summary metrics (all start rows)
    factors_series_top = df[alloc]
    max_start_top = len(factors_series_top) - (int(years) - 1) * int(step_rows)
    max_start_top = max(0, max_start_top)

    batch_results_top = []
    for s in range(0, max_start_top):
        final_bal_top = simulate_final_balance(
            factors=factors_series_top,
            years=int(years),
            starting_balance=float(starting_balance),
            annual_withdrawal=float(annual_withdrawal),
            start_row=int(s),
        ...
        if final_bal_top is None:
            continue
        batch_results_top.append(final_bal_top)

    if batch_results_top:
        starts_evaluated_top = len(batch_results_top)
        success_rate_top = sum(1 for b in batch_results_top if b > 0) / starts_evaluated_top
        m4, m5 = st.columns(2)
        m4.metric("Starts evaluated", f"{starts_evaluated_top:,}")
        m5.metric("% with Final Balance > 0", f"{success_rate_top*100:,.2f}%")
